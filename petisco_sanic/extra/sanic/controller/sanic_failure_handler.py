...
from petisco.extra.elastic_apm.is_elastic_apm_available import (
    is_elastic_apm_available,
)


def sanic_failure_handler(result: AnyResult, error_map: ErrorMap) -> NoReturn:
    ...
    if isinstance(domain_error, DomainError):
        ...
        logger.warning(
            f"sanic_failure_handler: {domain_error.__class__.__name__} is not a DomainError (warning "
            f"mapping error)"
        )
